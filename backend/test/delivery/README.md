...
That sending works end to end: the key is valid, `src/email.ts` builds a request SMTP2GO accepts, and SMTP2GO gets the mail to the recipient.
Nothing else belongs here.
The payload is pinned by `src/email.test.ts` and the wording of each mail by `test/api`, both without sending anything.

This is a check on a third party, not on a change, so it does not run per pull request.
`.github/workflows/mail.yml` runs it on Wednesdays and Fridays from August to February, which is the season, and it can be started by hand from the Actions tab at any time.
