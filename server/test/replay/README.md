...
It then builds the leaderboard of every league in that season and snapshots it.
...
and     the leaderboard is built for each league
then    it matches the committed snapshot
```
...
- **SMTP2GO.** `test/support/mail.ts` replaces `src/email/send.ts`, so mail can be asserted rather than swallowed.
