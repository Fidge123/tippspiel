  ...
  return {
    ...
    async team(id, asOf) {
      ...
      for (const division of divisions) {
        ...
        let snapshot = teamsBySnapshot.get(key);
        if (!snapshot) {
          const responses = await getJSON<any[]>(key);
          snapshot = new Map(responses.map((r) => [String(r.team.id), r]));
          teamsBySnapshot.set(key, snapshot);
        }
        const found = snapshot.get(String(id));
        if (found) {
          return found;
        }
      }
